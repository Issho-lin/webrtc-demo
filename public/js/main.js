...
function animateVirtualVideo() {
  ...
  virtualVideoContext.fillRect(0, 0, virtualVideoCanvas.width, virtualVideoCanvas.height);
  
  // 绘制用户名
  virtualVideoContext.fillStyle = '#FFFFFF';
  virtualVideoContext.font = '24px Arial';
  virtualVideoContext.textAlign = 'center';
  virtualVideoContext.fillText(username || '未登录', virtualVideoCanvas.width / 2, virtualVideoCanvas.height / 2 - 12);
  
  // 绘制时间
  const now = new Date();
  const timeString = now.toLocaleTimeString();
  virtualVideoContext.font = '20px Arial';
  virtualVideoContext.fillText(timeString, virtualVideoCanvas.width / 2, virtualVideoCanvas.height / 2 + 20);
...
function resetCallState() {
  ...
  updateCallStatus(selectedUser ? `准备与 ${selectedUser} 通话` : '选择一个用户开始通话');
}
